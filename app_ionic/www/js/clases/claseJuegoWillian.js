...
class JuegoWillian {
    ...
    constructor(canvasCJS, canvasBOX2D, nivel) {
        ...
        if (cantJuegos == 0) {
            ...
            console.log("no pued ehaber mas de un juego de williana a la vez, sino liman los canvas")
            return;
        }
    ...
    ganasteTodoElJuego(){
        ...
        if (this.agarroEstrella & this.cumplisteConTiempo) {
           ...
           audioW.gol(2)
        } else if (this.agarroEstrella == false & this.cumplisteConTiempo == true) {
           audioW.gol(2)
        } else if (this.agarroEstrella == false & this.cumplisteConTiempo == false) {
            audioW.gol(1)
        }
    ...
    gol() {
       ...
       if(this.nivel.mundo==5 && this.nivel.orden==9){
        ...
        console.log("Gol de: " + willian.ultimoTiro + " - fuera del area: " + willian.lePegoDeAfueraDelArea);
        usuario.meritos.registrarGol(willian.ultimoTiro, willian.lePegoDeAfueraDelArea);
    ...
    evaluarEstrellitasYPonerGanarPerder() {
        ...
        if (juegoW.tiempoTranscurrido > juegoW.nivel.tiempoObjetivo * 1000) this.cumplisteConTiempo = false
        ...
        usuario.registrarNivel(this.nivel.mundo, this.nivel.orden, true, this.cumplisteConTiempo, this.agarroEstrella);
    }
    ...
    restart() {
        ...
        cantJuegos--
        juegoW = new JuegoWillian('canvasCreateJs', 'canvasBox2d', window.nivelTemp);
        juegoW.setup();
    ...
    */
    ...
    skewLineasArea() {
        ...
        arco.circuloCentral.x = fl(arco.posXCirculoCentral - arco.circuloCentral.skewX)
        //console.log(arco.circuloCentral.skewX)

        let shouldBeHeight = arco.circuloCentral.nominalBounds.height;
        let isHeight = shouldBeHeight * Math.abs(Math.cos(deg2rad(arco.circuloCentral.skewX)));
        let heightRatio = shouldBeHeight / isHeight;
    ...
    ponerTiempoNivel() {
        ...
        if (this.pause == false && this.ganaste == false && this.perdiste == false && this.deltaTime > 0) {
    ...
    render(e) {
        ...
        if (this.pauseRender == false) {
            if (this.pause == false) {
                if (this.ganaste == false && this.perdiste == false) this.ponerTiempoNivel()
                if (!(debug > 0)) {
                    let aQuien;
            ...
            if (this.ganaste == false && this.perdiste == false) this.actualizarMapa()
            stage.update();
    ...
    calcularFramePisos() {
        ...
        for (let i = 0; i < 4; i++) {
            let frN = Math.floor(0.54 * -nivel.x / (Math.pow(i + 1, 1))) % this["piso" + i].totalFrames
            if (frN < 0) {
    ...
    movCamara(obj) {
        ...
        this.offsetXCamara -= Math.floor((this.offsetXCamara - this.offsetTarget) / 50)
        //ROTACION DE 'TODO'
        this.targetRotationCamara = -(this.offsetXCamaraAnterior - this.offsetXCamara) / 10
        ...
        if (this.ganaste || this.perdiste) {
            ...
            this.offsetTarget = 0
            nivel.x = fl(nivel.x - (nivel.x - (mitadX - obj.x)) / this.velocidadCamara)
        } else {
            ...
            if (this.targetNivelXAnterior == undefined) {
                this.targetNivelXAnterior = canvasWidth / 2 - obj.x + this.offsetXCamara
            } else {
            ...
            if ((((arco.x - pelota.getPos().x) < 1080 /* * this.escalaTarget*/) && (this.yaSePusoEnCamaraLentaSaltando == false) )|| ((arco.x - pelota.getPos().x) < 1800 && willian.tienePelota==1) ) {
                //this.escalaMax = 0.95
    ...
    moverFondo() {
        ...
        this.targetFondoX = (fl(mapear(porcRecorrido, -canvasWidth, this.anchoTotal + canvasWidth)) * 0.25) % anchoCachoFondo - anchoCachoFondo * 0.25;
    ...
    calcularEscala() {
        ...
        //interpolacion:
        this.escala = this.escala + (this.escalaTarget - this.escala) / (this.velocidadCamara*this.coefCamara)
    ...
    setup() {
        ...
        if(getQueryVariable("firebase")!=false) $("#botonpausa, #botonMenu").show();
        ...
        // WILLIAN y PELOTA
        pelota = new Pelota(this.nivel.pelota.x, this.nivel.pelota.y, this.nivel.pelota.fuerzaX, -this.nivel.pelota.fuerzaY, this.nivel.pelota.angularVel, this.nivel.pelota.aQueVelocidadDejaDeColisionar); //x,y,fuerzaX, -fuerzaY
        ...
        if (this.nivel.hasOwnProperty("monstruo") && this.nivel.monstruo.hasOwnProperty("x")) {
        ...
        $("#negro").show();
        createjs.Ticker.removeAllEventListeners()
        createjs.Ticker.addEventListener("tick", (e) => { this.step(e) });
    ...
    ponerMapa() {
        ...
        if (willian.getPos().x < this.itemMasAtras) this.itemMasAtras = willian.getPos().x
        if (pelota.getPos().x < this.itemMasAtras) this.itemMasAtras = pelota.getPos().x
        for (let i = 0; i < amigos.length; i++) {
            if (amigos[i].getPos().x < this.itemMasAtras) this.itemMasAtras = amigos[i].getPos().x
        }
        for (let i = 0; i < contrincantes.length; i++) {
            if (contrincantes[i].getPos().x < this.itemMasAtras) this.itemMasAtras = contrincantes[i].getPos().x
        }
        ...
        for (let i = 0; i < contrincantes.length; i++) {
            ...
            contrincantes[i].itemMapa = new librerias["lib_original"]["itemMapaEnemigo"+tipoEnemigo]();
            contrincantes[i].itemMapa.scaleX = contrincantes[i].itemMapa.scaleY = escalaItemsMapa
        ...
        setTimeout(()=>{
            if(this.nivel.arquero.aQVelDaReboteArriba<333) arquero.itemMapa.arriba.gotoAndStop(0)
            else if(this.nivel.arquero.aQVelDaReboteArriba<666 && this.nivel.arquero.aQVelDaReboteArriba>333) arquero.itemMapa.arriba.gotoAndStop(1)
            else if(this.nivel.arquero.aQVelDaReboteArriba>666 ) arquero.itemMapa.arriba.gotoAndStop(2)
            
            if(this.nivel.arquero.aQVelDaReboteMedio<333) arquero.itemMapa.medio.gotoAndStop(0)
            else if(this.nivel.arquero.aQVelDaReboteMedio<666 && this.nivel.arquero.aQVelDaReboteMedio>333) arquero.itemMapa.medio.gotoAndStop(1)
            else if(this.nivel.arquero.aQVelDaReboteMedio>666 ) arquero.itemMapa.medio.gotoAndStop(2)

            if(this.nivel.arquero.aQVelDaReboteAbajo<333) arquero.itemMapa.abajo.gotoAndStop(0)
            else if(this.nivel.arquero.aQVelDaReboteAbajo<666 && this.nivel.arquero.aQVelDaReboteAbajo>333) arquero.itemMapa.abajo.gotoAndStop(1)
            else if(this.nivel.arquero.aQVelDaReboteAbajo>666 ) arquero.itemMapa.abajo.gotoAndStop(2)
        },500)
        ...
        for(let i=0;i<this.mapa.children.length;i++){
            if(this.mapa.children[i]==arquero.itemMapa) this.mapa.children[i].alpha=0.8
    ...
    actualizarMapa() {
        ...
        for (let i = 0; i < amigos.length; i++) {
            ...
            amigos[i].itemMapa.x = (amigos[i].getPos().x - this.itemMasAtras) * ratioMapa

        }
        for (let i = 0; i < contrincantes.length; i++) {

            contrincantes[i].itemMapa.x = (contrincantes[i].getPos().x - this.itemMasAtras) * ratioMapa
    ...
    ponerONoEstrella() {
        ...
        let nNivel = buscarNivelPorNumeroDeMundoYOrden(this.nivel.mundo - 1, this.nivel.orden);
    ...
    posicionarNivelParaElArranque() {
        ...
        let dist = Math.floor(Math.abs(willian.getPos().x - pelota.getPos().x));
        this.escala = -0.000125 * dist + 0.95 * (0.000355 * pelota.getPos().y + 0.75)
        if (this.escala > this.escalaMax) this.escala = this.escalaMax
        if (this.escala < this.escalaMin) this.escala = this.escalaMin
    ...
    renderJugadores() {
        ...
        if (this.nivel.hasOwnProperty("monstruo") && monstruo != null && monstruo != undefined) {
            monstruo.render()
        }
    ...
    step() {
        ...
        if ((willian.tienePelota == 1 || willian.tienePelota == 2) && willian.tiro.angulo > 90 && willian.tiro.angulo < 270 && interfaz.pateando) {
        ...
        if (this.pause == false) world.Step(this.timeStep, ITERACIONES_POS, ITERACIONES_VEL);
            ...
            case b2Shape.e_circleShape:
                {
                    ...
                    // draw circle
                    context.moveTo(pos.x *escalaDebug+ r*escalaDebug+margenX, pos.y*escalaDebug+margenY);
                    for (var i = 0; i < segments; i++) {
                        var d = new b2Vec2(r * Math.cos(theta), r * Math.sin(theta));
                        var v = b2Math.AddVV(pos, d);
                        context.lineTo(v.x*escalaDebug+margenX, v.y*escalaDebug+margenY);
                        theta += dtheta;
                    }
                    context.lineTo(pos.x*escalaDebug + r*escalaDebug+margenX, pos.y*escalaDebug+margenY);

                    // draw radius
                    context.moveTo(pos.x*escalaDebug+margenX, pos.y*escalaDebug+margenY);
                    var ax = circle.m_R.col1;
                    var pos2 = new b2Vec2(pos.x*escalaDebug + r * ax.x*escalaDebug+margenX, pos.y *escalaDebug+ r * ax.y*escalaDebug+margenY);
                    context.lineTo(pos2.x*escalaDebug+margenX, pos2.y*escalaDebug+margenY);
                }
                break;
            case b2Shape.e_polyShape:
                {
                    ...
                    context.moveTo(tV.x*escalaDebug+margenX, tV.y*escalaDebug+margenY);
                    for (var i = 0; i < poly.m_vertexCount; i++) {
                        //   console.log(poly.m_vertices[i])
                        var v = b2Math.AddVV(poly.m_position, b2Math.b2MulMV(poly.m_R, poly.m_vertices[i]));
                        context.lineTo(v.x*escalaDebug+margenX, v.y*escalaDebug+margenY);
                    }
                    context.lineTo(tV.x*escalaDebug+margenX, tV.y*escalaDebug+margenY);
                }
                break;
        }
        context.stroke();
    ...
    filtroColisiones(shape1, shape2) {
        ...
        if ((shape1.m_userData instanceof Jugador && s2=="piso" )|| (shape2.m_userData instanceof Jugador && s1=="piso")) {
        ...
        if ((s1 == "caja" && s2 == "piso") || (s2 == "piso" && s1 == "caja")) {
            if(shape1.m_userData instanceof Caja) shape1.m_userData.eventoColisionPiso()
            else if(shape2.m_userData instanceof Caja) shape2.m_userData.eventoColisionPiso()
            return true;
        }

        if ((s1 == "arquero" && s2 == "caja") || (s2 == "arquero" && s1 == "caja")) {
            return false;
        }

        if ((shape1.m_userData instanceof Jugador && s2=="caja" )|| (shape2.m_userData instanceof Jugador && s1=="caja")) {
            if(shape1.m_userData instanceof Jugador){
        ...
        if (s1 == "pelota" || s2 == "pelota") {
            ...
            if ((s1 != "piso") && (s2 != "piso") && (s1 != "monstruo" && s2 != "monstruo")) {

                if ((pelota.noColisionaConNada) || (Math.abs(pelota.cuerpo.m_linearVelocity.x) > pelota.aQueVelocidadDejaDeColisionar)) return false;
            }
            else {
        ...
        if ((s1 == "monstruo" && s2 == "pelota") || (s1 == "pelota" && s2 == "monstruo")) {
            let m;
            if (s2 == "monstruo") {
                m = shape2.m_userData
            } else {
        ...
        if ((s1 == "willian" && s2 == "pelota") || (s1 == "pelota" && s2 == "willian")) {
            ...
            */
        ...
        } else if ((s1 == "arquero" && s2 == "pelota") || (s1 == "pelota" && s2 == "arquero")) {
        ...
        } else if ((s1 == "willian" && s2 == "contrincante") || (s1 == "contrincante" && s2 == "willian")) {
            ...
            return false //|| this.perdiste || this.ganaste;
        } else if ((s1 == "willian" && s2 == "arquero") || (s1 == "arquero" && s2 == "willian")) {
            return false //|| this.perdiste || this.ganaste;
        } else if ((s1 == "contrincante" && s2 == "arquero") || (s1 == "arquero" && s2 == "contrincante")) {
            return false// || this.perdiste || this.ganaste;
        } else if ((s1 == "amigo" && s2 == "willian") || (s1 == "willian" && s2 == "amigo")) {
            return false //|| this.perdiste || this.ganaste;
        } else if ((s1 == "amigo" && s2 == "contrincante") || (s1 == "contrincante" && s2 == "amigo")) {
            return false //|| this.perdiste || this.ganaste;
        } else if ((s1 == "amigo" && s2 == "arquero") || (s1 == "arquero" && s2 == "amigo")) {
            return false // || this.perdiste || this.ganaste;
        } else if ((s1 == "amigo" && s2 == "pelota") || (s1 == "pelota" && s2 == "amigo")) {
            let amigo;
            if (s2 == "amigo") {
        ...
        } else if ((s1 == "pelota" && s2 == "contrincante") || (s1 == "contrincante" && s2 == "pelota")) {
            ...
            //   console.log("piso!!")
            return true;
        } else if ((s1 == "arco" && s2 == "pelota") || (s1 == "pelota" && s2 == "arco")) {
        ...
        } else if ((s1 == "arcoAtras" && s2 == "pelota") || (s2 == "arcoAtras" && s1 == "pelota")) {
            arco.moverRed()
            return true;
        } else {
    ...
    ponerListenersABotonesGanarPerder() {
        ...
        this.ganarPerder.botMenu.addEventListener("click", juegoW.volverALaSeleccionDeNiveles)
        if (this.ganaste) this.ganarPerder.botSiguiente.addEventListener("click", () => this.siguienteNivel())
        else this.ganarPerder.botSiguiente.addEventListener("click", () => this.restart())
        this.ganarPerder.botRestart.addEventListener("click", () => this.restart())
    }
