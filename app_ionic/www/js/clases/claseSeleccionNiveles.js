...
class SeleccionNiveles {
  constructor(mundo) {
    ...
    $("#seleccionDeNiveles #tienda")
      .off()
      .click(() => {
        audioW.repro("boton");
        alert("no disponible");
      });
    $("#seleccionDeNiveles #config")
      .off()
      .click(() => {
        audioW.repro("boton");
        this.sacar();
  ...
  ponerBotonesComoVanSegunUsuario() {
    ...
    $(".nivel")
      .removeClass("estrella0 estrella1 estrella2 estrella3 bloqueado")
      .addClass("bloqueado")
      .off();
    let arrBotones = usuario.nivelesPorMundo(this.mundo);
    this.botonesNivelesAzules = $(
      "#seleccionDeNiveles #niveles div.nivel.especial"
    );
    if (arrBotones[0] > 0 && arrBotones[1] > 0 && arrBotones[2] > 0) {
      $(this.botonesNivelesAzules[0]).html("I");
      $(this.botonesNivelesAzules[0]).removeClass(
        "estrella0 estrella1 estrella2 estrella3 bloqueado"
      );
      $(this.botonesNivelesAzules[0]).off();
      $(this.botonesNivelesAzules[0]).click(() => {
        this.arrancarNivel("loco1");
      });
      let cantEstre = usuario.calcularCantEstrellasObtenidasPorNivel(
        usuario.niveles["locos"][this.mundo][0]
      );
      $(this.botonesNivelesAzules[0]).addClass("estrella" + cantEstre);
    } else {
      if (getQueryVariable("godMode") != false)
        $(this.botonesNivelesAzules[0])
          .off()
          .click(() => this.arrancarNivel("loco1"));
    }
    if (arrBotones[3] > 0 && arrBotones[4] > 0 && arrBotones[5] > 0) {
      let cantEstre = usuario.calcularCantEstrellasObtenidasPorNivel(
        usuario.niveles["locos"][this.mundo][1]
      );
      $(this.botonesNivelesAzules[1])
        .html("II")
        .removeClass("estrella0 estrella1 estrella2 estrella3 bloqueado")
        .off()
        .click(() => this.arrancarNivel("loco2"));
    ...
    if (arrBotones[6] > 0 && arrBotones[7] > 0 && arrBotones[8] > 0) {
      ...
      let cantEstre = usuario.calcularCantEstrellasObtenidasPorNivel(
        usuario.niveles["locos"][this.mundo][2]
      );
      $(this.botonesNivelesAzules[2])
        .html("III")
        .removeClass("estrella0 estrella1 estrella2 estrella3 bloqueado")
        .off()
        .click(() => this.arrancarNivel("loco3"));
  ...
  arrancarNivel(x) {
    ...
    if (
      juegoW != undefined &&
      juegoW instanceof Object &&
      juegoW.hasOwnProperty("fps")
    )
      juegoW.sacar();
    audioW.repro("boton");
    $("#cantEstrellas").hide();
  ...
  evaluarPonerTutoriales() {
    ...
    if (this.mundo == 0 && this.qNivel == 1) {
      ...
      if (usuario.tutoriales.patear == false) {
        ...
        this.ponerleClickAHTMLParaSaltearTutorial();
      }
  ...
  precargarCuerpos(nivel, cb) {
    if (nivel.hasOwnProperty("monstruo")) {
      ...
      traerComp.cargarComp("monstruos", () => {
        if (
          this.cual.camisetaWillian == this.cual.camisetaContrincantes ||
          (this.cual.hasOwnProperty("enemigos") &&
            this.cual.enemigos.length == 0) ||
          !this.cual.hasOwnProperty("enemigos")
        ) {
